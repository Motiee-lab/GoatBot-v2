...
const isProduction = NODE_ENV !== 'development'; // undefined (Render) or 'production'
const dirConfig = path.normalize(`${__dirname}/config${isProduction ? '.dev.json' : '.json'}`);
const dirConfigCommands = path.normalize(`${__dirname}/configCommands${isProduction ? '.dev.json' : '.json'}`);
const dirAccount = path.normalize(`${__dirname}/account${isProduction ? '.dev.txt' : '.txt'}`);
...
if (!config.author || config.author !== "April Manalo") {
        ...
        log.error("CRITICAL", `Expected: "April Manalo" | Got: "${config.author || 'MISSING'}"`);
        log.error("CRITICAL", "This bot requires the original author credit to function.");
        log.error("CRITICAL", "Bot is shutting down...");
        process.exit(1);
}
