...
function getTime(timestamps, format) {
        ...
        const timeZone = global.GoatBot?.config?.timeZone || "UTC";
        ...
        async function sendMessageError(err) {
                ...
                try {
                        if (api.sendMessageMqtt) {
                                return api.sendMessageMqtt(utils.getText("utils", "errorOccurred", err), event.threadID, event.messageID);
                        } else {
        ...
        return {
                send: async (form, callback) => {
                        try {
                                ...
                                if (api.sendMessageMqtt) {
                                        return api.sendMessageMqtt(form, event.threadID);
                                } else {
                ...
                unsend: async (messageID, callback) => {
                        try {
                                if (api.unsendMessageMqtt) {
                                        return api.unsendMessageMqtt(messageID, callback);
                                } else {
                                        return await api.unsendMessage(messageID, callback);
                                }
                        } catch (err) {
                                console.log("Unsend message error:", err?.message || err);
                        }
                },
                reaction: async (emoji, messageID, callback) => {
                        try {
                                ...
                                if (api.setMessageReactionMqtt) {
                                        return api.setMessageReactionMqtt(emoji, messageID, callback, true);
                                } else {
...
const drive = {
        parentID: "",
        async uploadFile() {
                throw new Error("Google Drive disabled - no credentials provided");
        },
        async deleteFile() {
                throw new Error("Google Drive disabled - no credentials provided");
        },
        getUrlDownload(id = "") {
                ...
                return `https://docs.google.com/uc?id=${id}&export=download&confirm=t`;
        },
        async getFile() {
                throw new Error("Google Drive disabled - no credentials provided");
        },
